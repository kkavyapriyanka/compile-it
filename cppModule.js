	...
	fs.writeFile( path  +  filename +'.cpp' , code  , function(err ){
		if(exports.stats)
		{
			...
			else
			{
				...
				if(envData.cmd === 'g++')
				{
                      console.log('INFO:'.green+"coming")
					//compile c code
					commmand = 'g++ ' + path + filename +'.cpp -o '+path + filename ;
					exec(commmand , function ( error , stdout , stderr ){
						if(error)
						{  console.log('INFO:'.green+"error")
							if(exports.stats)
							{
								console.log('INFO: '.green + filename + '.cpp1 contained an error while compiling');
							}
							...
							exec( tempcommand , function ( error , stdout , stderr )
							{
								...
								if(error)
								{
									...
									else
									{
										if(exports.stats)
										{
											console.log('INFO: '.green + filename + '.cpp2 contained an error while executing');
										}
								...
								{  console.log('INFO:'.green+"coming")
									if(progNotFinished){
										...
										if(exports.stats)
										{
											console.log('INFO: '.green + filename + '.cpp3 successfully compiled and executed !');
										}
								...
								exec( tempcommand + '<' + inputfile , function( error , stdout , stderr ){
									if(error)
									{
										...
										else
										{
											if(exports.stats)
											{
												console.log('INFO: '.green + filename + stderr);
											}
											var out = { error : stderr};
											fn(out);
										}
